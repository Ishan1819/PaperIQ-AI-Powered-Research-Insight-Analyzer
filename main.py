...
# Split text into paragraph chunks (200 chars)
...
def extract_topic_from_query(query: str) -> str:
    ...
    # Removing question words and common phrases
...
# format the response based on query type and match type.
def format_response(query: str, paragraph: str, match_type: str, topic: str) -> str:
    ...
    # Handle features queries
    elif is_features_query:
        # Looks for "Key features" section in paragraph
        features_match = re.search(r'key features.*?:', paragraph, re.IGNORECASE)
        
        if features_match:
            # Extract everything after "Key features:"
    ...
    # handle "how" queries
        ...
        # return process/mechanism explanation
        ...
        # for is_pattern, find and return the definition sentence
...
def retrieve_paragraph_answer(user_query: str, sections: dict, full_text: str, threshold: float=0.15) -> str:
    ...
    if best_score > threshold:
        ...
        # Format and generate natural response
...
if uploaded_file:
    ...
    with st.expander("Document Structure"):
        for section, paragraphs in st.session_state.sections.items():
            st.write(f"**{section}**: {len(paragraphs)} paragraph(s)")

    if st.button("Generate Summary"):
        summary = simple_summarize(st.session_state.full_text, summary_ratio=0.3)
        st.subheader("Document Summary")
        st.write(summary)

    query = st.text_input("Ask a question about the document:")
    if query:
        answer = retrieve_paragraph_answer(query, st.session_state.sections, st.session_state.full_text)
